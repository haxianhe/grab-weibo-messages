...
class Weibo(object):
    ...
    def print_user_info(self):
        ...
        print('+' * 100)
        print(u'位置信息')
        print(u'位置id：%s' % self.user['containerid'])
        print(u'位置名称：%s' % self.user['title'])
        # gender = u'女' if self.user['gender'] == 'f' else u'男'
        # print(u'性别：%s' % gender)
        print(u'微博数：%d' % self.user['total'])
        print(u'点赞数：%d' % self.user['attitudes_count'])
    ...
    def get_one_page(self, page):
        ...
        try:
            ...
            if js['ok']:
                if len(js['data']['cards']) > 1:
                    weibos = js['data']['cards'][3]['card_group'];
                else:
    ...
    def get_filepath(self, type):
        ...
        try:
            ...
            )[0] + os.sep + 'weibo' + os.sep + self.user['title']
            if type == 'img':
